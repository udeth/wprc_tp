...
class fileUpload{
	var $saveName;// 保存名
	var $savePath;// 保存路径
	var $fileFormat = array('gif','jpg','png');// 文件格式&MIME限定
	var $overwrite = 0;// 覆盖模式
	var $maxSize = 0;// 文件最大字节
	var $ext;// 文件扩展名
	var $thumb = 0;// 是否生成缩略图
	var $thumbWidth = 200;// 缩略图宽
	var $thumbHeight = 200;// 缩略图高
			...
			if (($originalHeight < $this->thumbHeight 
				...
				*/
				...
				if($originalWidth>$originalHeight){
					$yuanX=($originalWidth-$originalHeight)/2;
					$yuanY=0;
					$originalWidth=$originalHeight;
				}else{
					$yuanY=($originalHeight-$originalWidth)/2;
					$yuanX=0;
				...
				if ( !imagecopyresampled($createdThumb, $Original, 0, 0, $yuanX, $yuanY, 
