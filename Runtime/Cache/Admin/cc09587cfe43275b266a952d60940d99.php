...
<body>
	...
	<div class="cf">
		<div class="fl">
            <a class="btn" href="<?php echo U('add');?>">测试新增</a>
            <button class="btn ajax-post confirm" url="<?php echo U('delete');?>" target-form="ids">删 除</button>
        </div>

        <span>&nbsp;&nbsp;
        	总网评数：<?php echo count($_list); ?>；
			通过审核数：<?php echo ($countG); ?>；
			不通过审核数：<?php echo ($countB); ?>；
			未审核：<?php echo ($countW); ?>；
		...
		<div class="search-form fr cf">
			<div class="sleft">
				<input id="searchtext" type="text" name="name" class="search-input" value="<?php echo I('name');?>" placeholder="请输入用户名、产品名、店名或网址">
    ...
    <tbody>
		<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><?php echo ($vo["product_category_name"]); ?></td>
			<td><?php echo ($vo["store_name"]); ?></td>
			<td><?php echo ($vo["address"]); ?></td>
			<td><?php echo ($vo["time"]); ?></td>
			<?php if($vo["checked"] == 0): ?><td class="unchecked">
			<?php else: ?><td><?php endif; ?>
			<?php if($vo['checked']==1){echo "审核通过";}elseif($vo['checked']==2){echo "未通过";}else{echo "未审核";} ?></td>
			<td>
                <?php if(($vo["checked"]) == "0"): ?><a href="<?php echo U('edit?id='.$vo['id']);?>">审核</a><?php endif; ?>
                
                <a href="<?php echo U('detail?id='.$vo['id']);?>">详情</a>
				<a href="<?php echo U('delete?id='.$vo['id']);?>" class="confirm ajax-get">删除</a>
            </td>
    ...
    //导航高亮
    highlight_subnav('<?php echo U('Review/index');?>');
	</script>
